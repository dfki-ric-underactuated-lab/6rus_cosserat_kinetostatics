...
def main(p_ee, angles, init_guess, RPY):
    ...
    ee_mass = 1e-12#0.000001 #Kg #neglible mass 0.000001
    g = np.array([0,0,-9.81]) 
...
#     num_points_on_circle = 150
#     num_points_inside_circle = 150
#     # z_height = 0.5  # Constant z-height
#     totalpoints =[]
#     # Circle parameters
    
#     # Generate random points on and inside the circle
#     for i, j in zip(np.arange(0.4, 0.72, 0.02), circle_radius):
#         points_inside_circle = generate_random_points_inside_circle(j, num_points_inside_circle, i)
#         totalpoints.append(points_inside_circle)
...
# Now, x, y, and z contain the generated random points in arrays.
# print(points[0])
p_ee = points
print(len(p_ee))
fig = plt.figure()
...
# plt.show()
# print(np.arange(0.4,0.72,0.02))
# p_ee = np.array([[0,0,0.4],[0,0,0.5]])

# x = np.linspace(0,0.35,sample)
# z_rot = np.linspace(0, 90, 100)
# sample = len(z_rot)
q1_vec = np.empty(6)
q2_vec = np.empty(6)
q3_vec = np.empty(6)
...
while i<len(p_ee):
    ...
    # p_ee = p_ee = np.array([0, 0, 0.4])
    # angles = np.array([np.deg2rad(0),np.deg2rad(0),np.deg2rad(z_rot[i])])
...
save_excel('/home/dfki.uni-bremen.de/vrodrigues/6_rus_Stewart_platform/Simulation/Flexible_links/Cosserat_rod_theory/excel_files/PACOMA_Rotation_EE_workspace_ROD.xlsx',
