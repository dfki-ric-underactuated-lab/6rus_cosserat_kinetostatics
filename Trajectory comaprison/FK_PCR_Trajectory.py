...
def Forward_Kinetostatic(qm, init_guess):
...
def FK_plots():
    ...
    distances = np.linalg.norm(dfIK5C[dfIK5C.columns[7:10]].values - dfFK5C[dfFK5C.columns[4:7]].values, axis=1)
    ...
    ax.scatter(x,y,z,label='Reference trajectory',c='g',linewidth=3.0)
    # ax.scatter(dfIK5C[7], dfIK5C[8], dfIK5C[9], label='Reference EE position',marker="o", c='r', alpha=0.5)
    ax.scatter(dfFK5C[4], dfFK5C[5], dfFK5C[6], label='FK solution', s = 80,facecolors='none', edgecolors='k')
