...
def main(p_ee, angles, init_guess, RPY):
    ...
    ee_mass = 1#1e-12 #Kg #neglible mass 0.000001
    g = np.array([0,0,-9.81]) 
    ...
    return sol, restrack[-1], total_time, r, Mm, p0track[-6:], ptrack[-18:], Ree, pLtrack[-6:], p_ee, F
...
#     image_dir = "/home/dfki.uni-bremen.de/vrodrigues/6_rus_Stewart_platform/Simulation/Flexible_links/Cosserat_rod_theory/excel_files/Trajectories/Images"

#     # Output video file
#     output_video = "/home/dfki.uni-bremen.de/vrodrigues/6_rus_Stewart_platform/Simulation/Flexible_links/Cosserat_rod_theory/excel_files/Trajectories/video.mp4"
...
#     out = cv2.VideoWriter(output_video, fourcc, 0.5, (width, height))
...
def plotting(counter):
    ...
    output_dir = "/home/dfki.uni-bremen.de/vrodrigues/6_rus_Stewart_platform/Simulation/Flexible_links/Cosserat_rod_theory/excel_files/Trajectories/Images"
    ...
    # Reshape the DataFrame to get 6 sets of (3, N) coordinates
    # print(f"ptrack_type: {type(ptrack)}")

    coordinates = ptrack.reshape(6, 3, 100)
    # print(f"ptrack_shape: {coordinates.shape}")
    if NormalPlot:
         # # Create a 3D plot
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')

        ax.plot(xc,yc,zc, label='Circular Trajectory', linewidth=2)
        # # Plot each set of (3, 3) coordinates
...
saveData_bool=False #saving data to an excel file 
NormalPlot = True
...
def circTraj():
    ...
    # print(f'shape of circle points :{points_on_circle.shape}')
...
P_ee = np.column_stack((xh,yh,zh))
# P_ee = np.array([[0.25,0,0.4],[-0.25,0,0.4]])
...
while i<len(P_ee):
    ...
    #excuting the main function
    optimised_states, restrack, total_time, r, Mm, p0, ptrack, Ree, pLtrack, p_ee, F = main(P_ee[i], angles, init_guess, RPY=True)
    # print(f"ptrack_shape: {ptrack.shape}")
    linAct_error.append(np.concatenate(([total_time], F, [optimised_states[24]],[optimised_states[27]],[optimised_states[30]],[optimised_states[33]],[optimised_states[36]],[optimised_states[39]], P_ee[i],restrack.flatten())))
    # plotting(i)
    # print(f"linAct_error: {linAct_error}")
    # init_guess = optimised_states
    i+=1

save_excel('/home/dfki.uni-bremen.de/vrodrigues/6_rus_Stewart_platform/Simulation/Flexible_links/Cosserat_rod_theory/excel_files/Trajectories/PACOMA_IK_trajec_circ_3_noinitial_r025.xlsx',
