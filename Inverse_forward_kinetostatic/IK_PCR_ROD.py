...
def Inverse_Kinetostatic(p_ee, R_ee, init_guess):
 ...
 #####################################################################################################################
    ...
    #orientation of the end-effector platform
    R_ee = RPY_to_matrix(R_ee[0], R_ee[1], R_ee[2])
    #Homogeneous transformation matrix--> EE to base frame coordinate
    O_Tee_EE = np.hstack((np.vstack((R_ee, np.zeros((1,3)))), np.vstack(((np.reshape(p_ee,(3,1))), 1))))
    ...
    RC = [R_ee @ Rz_120, R_ee @ Rz_120.T, R_ee @ Rz_120.T, R_ee @ np.eye(3), R_ee @ np.eye(3), R_ee @ Rz_120]
...
####################################################################################################################################### 
    ...
    return sol, r, Mm, p0track, ptrack, R_ee, pLtrack[-6:]


def plotting(path_excel, path_img):
    '''
...
optimised_states, r,  Mm, p0track, ptrack, R_ee, pLtrack = Inverse_Kinetostatic(p_ee, R_ee, init_guess)
qrad =[] 
qdeg = []
...
if saveData_bool: 
        save_excel('path-to-excel-file.xlsx',
                   [ptrack, p0track])
if plot_bool: 
    plotting('path-to-excel-file.xlsx', 
            'path-to-image.pdf')
